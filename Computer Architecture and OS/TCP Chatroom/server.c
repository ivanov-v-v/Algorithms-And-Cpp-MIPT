...
int main(int argc, char **argv) {
    ...
    printf("login: root\npassword: ");
    scanf("%s", message);
    clients[0] = (struct user*) malloc(sizeof(struct user));
...
void* connection_handler(void* socket_desc) {
    ...
    if (!already_registered_flag) {
        ...
        for (size_t i = 0; i < MAX_CLIENTS; ++i) {
            if (clients[i] == NULL) {
                curr_uid = i;
                clients[i] = (struct user*) malloc(sizeof(struct user));
        ...
        if (read_size < 2 || read_size > 31) {
            ...
            if (already_registered_flag) {
                if (!strcmp(clients[curr_uid]->password, client_message)) {
                    ...
                    for (size_t i = 0; i < MAX_SESSIONS; ++i) {
                        if (sessions[i] == NULL) {
                            sessions[i] = (struct user_session*) malloc(sizeof(struct user_session));
